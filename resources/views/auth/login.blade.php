...
<div class="w-full max-w-sm px-6">
    <div class="text-center mb-8">
        ...
        <p class="text-zinc-500 text-sm mt-1">Gestion financière</p>
    </div>

    @if(isset($error) && $error)
        <div class="bg-red-900/30 border border-red-800 text-red-400 rounded-lg px-4 py-3 mb-6 text-sm">
            {{ $error }}
        </div>
    @endif
    ...
    <div class="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
        <p class="text-zinc-400 text-sm text-center mb-6">
            Accès réservé aux membres <strong class="text-white">@groupe-speed.cloud</strong>
        </p>
        <a href="/login?force=1" class="flex items-center justify-center gap-3 w-full bg-white hover:bg-zinc-100 text-zinc-900 font-medium py-2.5 px-4 rounded-lg transition-colors text-sm">
