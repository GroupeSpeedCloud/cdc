...
?>
<aside id="sidebar" class="w-64 flex-shrink-0 bg-slate-900 flex flex-col z-30 h-screen">
  ...
  <div class="flex items-center justify-between px-5 py-4 border-b border-slate-800">
    <div class="flex items-center gap-2.5">
      ...
      <div>
        ...
        <p class="text-slate-400 text-xs leading-tight mt-0.5">Groupe Speed Cloud</p>
      </div>
    </div>
    <button id="sidebar-close" class="lg:hidden p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 transition-colors">
  ...
  <nav class="flex-1 px-3 py-4 overflow-y-auto space-y-0.5">
    <p class="px-3 pb-2 text-xs font-semibold text-slate-500 uppercase tracking-widest">Navigation</p>
    ...
    <p class="px-3 pt-5 pb-2 text-xs font-semibold text-slate-500 uppercase tracking-widest">Exports</p>
    <?php foreach ([
    ...
    <a href="<?= APP_URL . $href ?>" <?= str_contains($href,'pdf') ? 'target="_blank"' : '' ?>
       class="flex items-center gap-3 px-3 py-2 rounded-xl text-sm text-slate-400 hover:text-white hover:bg-slate-800 transition-colors">
  ...
  <div class="px-4 py-4 border-t border-slate-800">
    ...
    <div class="flex items-center gap-3 mb-3">
      ...
      <div class="min-w-0">
        ...
        <p class="text-xs text-slate-400 truncate"><?= htmlspecialchars($u['email'] ?? '', ENT_QUOTES, 'UTF-8') ?></p>
      </div>
    </div>
    <form method="POST" action="<?= APP_URL ?>/logout">
      <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
      <button type="submit" class="w-full flex items-center gap-2 px-3 py-2 text-sm text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl transition-colors">
